               ...
               (case (first form)
                 ((prod :rule) (destructuring-bind (prod name locals body) form
                                 `(defmethod ,(intern (string-upcase name) package) ((o ,*ometa-compiler-target*) arg)
                                    (let ((*start* (input-position (input-stream o))))
                 ...
                 ((assign :assign :set) `(let ((v ,(rec (if (equal (third form) `(:app :anything)) `(:loadarg) (third form)))))
                                           ...
                                           v))
                 ((action :action :act) `(let ((*end* (input-position (input-stream o))))
                                           (symbol-macrolet ((*range* (cons *start* *end*)))
                                             ,(if (stringp (second form))
                                                  (read-from-string (second form))
                                                  (rec (second form))))))
